...
onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
        ...
        if(deepLinkUsername && !path.endsWith('chat.html')) { window.location.replace(`chat.html?link=${deepLinkUsername}`); return; }
...
window.Fluxgram.profile = {
    ...
    openUserEdit: () => {
        ...
        if(State.userData.photoURL) {
            preview.src = State.userData.photoURL;
            preview.classList.remove('hidden'); text.classList.add('hidden');
        } else {
    ...
    openChatEdit: () => {
        ...
        if(d.photoURL) {
            preview.src = d.photoURL; preview.classList.remove('hidden'); text.classList.add('hidden');
        } else {
            preview.classList.add('hidden'); text.classList.remove('hidden'); text.innerText = d.name.charAt(0).toUpperCase();
        }
        ...
        State.unsubChats = onSnapshot(q, async (snapshot) => {
            ...
            for (const data of chatDocs) {
                ...
                try {
                    ...
                    list.innerHTML += `<div class="chat-item" onclick="window.location.href='chat.html?uid=${otherUid}'"><div class="avatar">${Utils.renderAvatarHTML(otherUser.photoURL, otherUser.username)}</div><div class="chat-info"><div class="c-name-row"><div class="c-name">${otherUser.username}</div><div class="c-time">${timeStr}</div></div><div class="c-msg-row"><div class="c-msg">${data.lastMessage || ''}</div>${unread}</div></div></div>`;
                } catch(err) {}
            }
        });
    }
            ...
            if (existingChatId) {
                ...
                onSnapshot(doc(db, "users", otherUid), (d) => {
                    if(d.exists()) {
                        ...
                        document.getElementById('chat-avatar').innerHTML = Utils.renderAvatarHTML(u.photoURL, u.username);
