                ...
                container.innerHTML += `
                    <div class="msg-row ${isMe ? 'msg-tx' : 'msg-rx'}">
                        <div class="msg-bubble" onclick="Fluxgram.chat.showDeleteMenu('${msgId}', '${msg.senderId}')">
                            ${senderNameHTML}${contentHTML}<div class="msg-meta">${timeStr}</div>
                        </div>
                    </div>`;
            });
...
window.Fluxgram.call = {
    startCall: async (type) => {
        ...
        try {
            ...
            onSnapshot(callDoc, (snapshot) => { 
                ...
                if (!pc.currentRemoteDescription && data?.answer) { pc.setRemoteDescription(new RTCSessionDescription(data.answer)); State.startTime = Date.now(); } 
                if(data?.status === 'ended') window.Fluxgram.call.endCallLocal(true); 
            });
    ...
    toggleVideo: () => { 
        ...
        if(videoTrack) { 
            ...
            btn.innerHTML = videoTrack.enabled ? '<i class="fas fa-video"></i><br><span style="font-size:0.7rem;">Video</span>' : '<i class="fas fa-video-slash" style="color:var(--danger);"></i><br><span style="font-size:0.7rem;">Video</span>'; 
        } 
    },
    toggleSpeaker: () => {
